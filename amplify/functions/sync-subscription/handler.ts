...
export const handler: AppSyncResolverHandler<Args, Result> = async (event) => {
  ...
  const subscriptionStatus = isFoundingMember || event.arguments.entitlementActive ? 'active' : 'inactive';
  const subscriptionEndDate = event.arguments.entitlementActive
    ? event.arguments.expirationDate ?? null
    : null;
  const subscriptionProvider = isFoundingMember && !event.arguments.entitlementActive
    ? 'founding_member'
    : 'revenuecat';
